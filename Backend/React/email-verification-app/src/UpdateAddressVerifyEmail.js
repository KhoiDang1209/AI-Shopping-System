...
function UpdateAddressVerifyEmail() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response1 = await axios.post('http://localhost:8000/postupdateAddress', userData);  // Sending full user data
            console.log('Registration successful:', response1.data);
            setMessage("Redirecting to Homepage...");
            const response2 = await axios.post('http://localhost:8000/getUserInfoByEmail', { email: userData.email });  // Sending full user data
            console.log(response2.data)
            navigate('/Homepage', { state: { userData: { ...response2.data } } });
        } catch (error) {
...
export default UpdateAddressVerifyEmail;
